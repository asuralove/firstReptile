...
import sanicdb #对异步mysql的封装

from ReptileFrame.urlpool import UrlPool
from ReptileFrame import functions as fn, config


class NewsCrawlerAsync:
    def __init__(self, name):
        self._workers = 0  #协程计数器
        self._workers_max = 30
        self.logger = fn.init_file_logger(name+ '.log')

        self.urlpool = UrlPool(name)

        self.loop = asyncio.get_event_loop() #获得事件循环
