...
class UrlPool:
    ...
    def __init__(self, pool_name):
        ...
        self.waiting = {}  # {host: set([urls]), } 按host分组，记录等待下载的URL 按照url的host进行分类，相同host的url放到一起，在取出时每个host取一个url，尽量保证每次取出的一批url都是指向不同的服务器的，这样做的目的也是为了尽量减少对抓取目标服务器的请求压力。
        self.pending = {}  # {url: pended_time, } 记录已被取出（self.pop()）但还未被更新状态（正在下载）的URL pended_time取出时的时间戳
        ...
        self.hub_refresh_span = 0 #时间间隔
        self.load_cache()

    #class UrlPool消除时自动调用
    def __del__(self):
        self.dump_cache()
